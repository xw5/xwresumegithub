<template>
  <div v-if="isShow" :style="{opacity: isOpa ? 1 : 0}" class="w-full h-screen fixed inset-0 z-50 bg-[rgba(0,0,0,.8)] flex flex-col justify-center items-center clock-wrap">
    <div class="clock-container relative" :style="{transform: `rotate(${rotateDeg}deg) scale(${scaleSize})`}">
      ...
      <div class="text-[rgba(255,255,255,.26)] text-[40px] absolute bottom-0 right-[-68px]">{{ ampm }}</div>
    ...
    <span @click="hide" class="iconfont absolute right-[5px] cursor-pointer text-white close" :class="rotateDeg === 90 ? 'bottom-[5px]' : 'top-[5px]'">&#xe61a;</span>
